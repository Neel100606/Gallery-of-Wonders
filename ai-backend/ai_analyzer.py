...
# --- IMAGE MODELS (Loaded once when the server starts) ---
print("Loading Image AI models...")
...
# --- TEXT MODELS (Loaded once when the server starts) ---
print("Loading Text AI models...")
...
def analyze_image_with_ai(image_url, top_k_tags=5):
    """Analyzes an image from a URL to generate a caption and tags."""
    try:
        ...
        return {"caption": "Could not process image.", "tags": []}

    # --- Generate Caption using BLIP ---
    inputs = blip_processor(image, return_tensors="pt")
    out = blip_model.generate(**inputs, max_new_tokens=50) # Limit caption length
    ...
    predicted_tags = [vit_model.config.id2label[idx.item()].split(',')[0] for idx in top_k.indices]
            
    return {"caption": generated_caption, "tags": predicted_tags}


def analyze_text_with_ai(text_content, top_k_tags=5):
    """Analyzes text to generate a summary (caption) and tags."""
    if not text_content or len(text_content.split()) < 20:
        return {"caption": "Content too short to analyze.", "tags": []}
